...
function md5(data: BufferSource, offset: number): ArrayBuffer {
  ...
  for (i = 0; i < length; ++i) padded[i] = view[offset++];
 ...
 */
export async function createOpenSSLPrivateKey(
  aesKeyLength: number,
  ...
  const key = await crypto.subtle.importKey("raw", openSSLKey, algorithm, false, [
    "encrypt",
    "decrypt",
  ]);
