    ...
    */
			value: function fromSchema(schema) {
				...
				this.requestList = Array.from(asn1.result["TBSRequest.requests"], function (element) {
					return new Request({ schema: element });
				});
   ...
   */
    ...
    */
			...
			value: function createForCertificate(certificate, parameters) {
				...
				sequence = sequence.then(function () {
					_this58.tbsRequest = new TBSRequest({
    ...
    */
			...
			value: function sign(privateKey, hashAlgorithm) {
				...
				return crypto.sign(defParams.algorithm, privateKey, new Uint8Array(tbs)).then(function (result) {
					...
					_this59.optionalSignature.signature = new BitString({ valueHex: result });
				}, function (error) {
					return Promise.reject("Signing error: " + error);
				});
				//endregion
    ...
    */
	...
	function parseOCSPReq() {
		...
		for (var _i11 = 0; _i11 < ocspReqSimpl.tbsRequest.requestList.length; _i11++) {
			var _row = requestsTable.insertRow(requestsTable.rows.length);
			var _cell = _row.insertCell(0);
			_cell.innerHTML = bufferToHexCodes(ocspReqSimpl.tbsRequest.requestList[_i11].reqCert.serialNumber.valueBlock.valueHex);
		}
		//endregion 

		//region Put information about request extensions 
		if ("requestExtensions" in ocspReqSimpl.tbsRequest) {
			for (var _i12 = 0; _i12 < ocspReqSimpl.tbsRequest.requestExtensions.length; _i12++) {
				var _row2 = extensionTable.insertRow(extensionTable.rows.length);
				var _cell2 = _row2.insertCell(0);
				_cell2.innerHTML = ocspReqSimpl.tbsRequest.requestExtensions[_i12].extnID;
			}
	...
	//*********************************************************************************
	context("OCSP Request Complex Example", function () {
		it("Create And Parse OCSP Request", function () {
			return createOCSPReqInternal().then(function () {
				var asn1 = fromBER(ocspReqBuffer);
