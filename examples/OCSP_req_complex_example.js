...
(function () {
  ...
  function createOCSPReqInternal() {
    //region Initial variables
    var ocspReqSimpl = new OCSPRequest(); //endregion
    ...
    ocspReqBuffer = ocspReqSimpl.toSchema(true).toBER(false); //endregion
  ...
  function createOCSPReq() {
    return Promise.resolve().then(() => createOCSPReqInternal()).then(() => {
      ...
      resultString = "".concat(resultString).concat(formatPEM(toBase64(arrayBufferToString(ocspReqBuffer))));
  ...
  function parseOCSPReq() {
    ...
    if ("requestorName" in ocspReqSimpl.tbsRequest) {
      switch (ocspReqSimpl.tbsRequest.requestorName.type) {
        ...
        case 7:
          // iPAddress
          {
            var view = new Uint8Array(ocspReqSimpl.tbsRequest.requestorName.value.valueBlock.valueHex); // noinspection InnerHTMLJS

            document.getElementById("ocsp-req-name-simpl").innerHTML = "".concat(view[0].toString(), ".").concat(view[1].toString(), ".").concat(view[2].toString(), ".").concat(view[3].toString());
            document.getElementById("ocsp-req-nm-simpl").style.display = "block";
          }
          break;
    ...
    for (var _i34 = 0; _i34 < ocspReqSimpl.tbsRequest.requestList.length; _i34++) {
      var _row = requestsTable.insertRow(requestsTable.rows.length);

      var _cell = _row.insertCell(0); // noinspection InnerHTMLJS


      _cell.innerHTML = bufferToHexCodes(ocspReqSimpl.tbsRequest.requestList[_i34].reqCert.serialNumber.valueBlock.valueHex);
    } //endregion 
    //region Put information about request extensions 


    if ("requestExtensions" in ocspReqSimpl.tbsRequest) {
      for (var _i35 = 0; _i35 < ocspReqSimpl.tbsRequest.requestExtensions.length; _i35++) {
        var _row2 = extensionTable.insertRow(extensionTable.rows.length);

        var _cell2 = _row2.insertCell(0); // noinspection InnerHTMLJS


        _cell2.innerHTML = ocspReqSimpl.tbsRequest.requestExtensions[_i35].extnID;
      }
  ...
  context("OCSP Request Complex Example", () => {
    it("Create And Parse OCSP Request", () => {
      return createOCSPReqInternal().then(() => {
        var asn1 = fromBER(ocspReqBuffer); // noinspection JSUnusedLocalSymbols
