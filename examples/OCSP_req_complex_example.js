...
(function () {
  ...
  */
   ...
   */
		fromSchema(schema) {
			//region Clear input data first
			clearProps(schema, ["reqCert", "singleRequestExtensions"]);
			//endregion
			...
			if ("singleRequestExtensions" in asn1.result) this.singleRequestExtensions = Array.from(asn1.result.singleRequestExtensions.valueBlock.value, element => new Extension({ schema: element }));
			//endregion
		}
		//**********************************************************************************
		/**
  ...
  */
   ...
   */
		fromSchema(schema) {
			//region Clear input data first
			clearProps(schema, ["TBSRequest", "TBSRequest.version", "TBSRequest.requestorName", "TBSRequest.requests", "TBSRequest.requestExtensions"]);
			//endregion
			...
			if ("TBSRequest.requestorName" in asn1.result) this.requestorName = new GeneralName({ schema: asn1.result["TBSRequest.requestorName"] });

			this.requestList = Array.from(asn1.result["TBSRequest.requests"], element => new Request({ schema: element }));

			if ("TBSRequest.requestExtensions" in asn1.result) this.requestExtensions = Array.from(asn1.result["TBSRequest.requestExtensions"].valueBlock.value, element => new Extension({ schema: element }));
			//endregion
		}
		//**********************************************************************************
		/**
  ...
  */
   ...
   */
    ...
    */
			...
			return new Sequence({
				name: names.blockName || "",
				value: [AlgorithmIdentifier.schema(names.signatureAlgorithm || {}), new BitString({ name: names.signature || "" }), new Constructed({
					optional: true,
					idBlock: {
						tagClass: 3, // CONTEXT-SPECIFIC
						tagNumber: 0 // [0]
   ...
   */
		fromSchema(schema) {
			//region Clear input data first
			clearProps(schema, ["signatureAlgorithm", "signature", "certs"]);
			//endregion
			...
			if ("certs" in asn1.result) this.certs = Array.from(asn1.result.certs, element => new Certificate({ schema: element }));
			//endregion
		}
		//**********************************************************************************
		/**
  ...
  */
   ...
   */
		fromSchema(schema) {
			//region Clear input data first
			clearProps(schema, ["tbsRequest", "optionalSignature"]);
			//endregion
			...
			if ("optionalSignature" in asn1.result) this.optionalSignature = new Signature({ schema: asn1.result.optionalSignature });
			//endregion
		}
		//**********************************************************************************
		/**
   ...
   */
	...
	function createOCSPReqInternal() {
		//region Initial variables
		var ocspReqSimpl = new OCSPRequest();
		//endregion
		...
		ocspReqSimpl.tbsRequest.requestList = [new Request({
			reqCert: new CertID({
				...
				issuerNameHash: new OctetString({ valueHex: fictionBuffer }),
				issuerKeyHash: new OctetString({ valueHex: fictionBuffer }),
				serialNumber: new Integer({ valueHex: fictionBuffer })
			})
		})];

		ocspReqSimpl.tbsRequest.requestExtensions = [new Extension({
			extnID: "1.3.6.1.5.5.7.48.1.2", // ocspNonce
			extnValue: new OctetString({ valueHex: fictionBuffer }).toBER(false)
		})];
		//endregion
	...
	function parseOCSPReq() {
		...
		var asn1 = fromBER(ocspReqBuffer);
		var ocspReqSimpl = new OCSPRequest({ schema: asn1.result });
		//endregion 
		...
		if ("requestorName" in ocspReqSimpl.tbsRequest) {
			switch (ocspReqSimpl.tbsRequest.requestorName.type) {
				...
				case 7:
					// iPAddress
					{
						var view = new Uint8Array(ocspReqSimpl.tbsRequest.requestorName.value.valueBlock.valueHex);
				...
				case 4:
					// directoryName
					{
						...
						for (var i = 0; i < ocspReqSimpl.tbsRequest.requestorName.value.typesAndValues.length; i++) {
							var typeval = rdnmap[ocspReqSimpl.tbsRequest.requestorName.value.typesAndValues[i].type];
							if (typeof typeval === "undefined") typeval = ocspReqSimpl.tbsRequest.requestorName.value.typesAndValues[i].type;

							var subjval = ocspReqSimpl.tbsRequest.requestorName.value.typesAndValues[i].value.valueBlock.value;
		...
		for (var _i18 = 0; _i18 < ocspReqSimpl.tbsRequest.requestList.length; _i18++) {
			var _row = requestsTable.insertRow(requestsTable.rows.length);
			var _cell = _row.insertCell(0);
			// noinspection InnerHTMLJS
			_cell.innerHTML = bufferToHexCodes(ocspReqSimpl.tbsRequest.requestList[_i18].reqCert.serialNumber.valueBlock.valueHex);
		}
		//endregion 

		//region Put information about request extensions 
		if ("requestExtensions" in ocspReqSimpl.tbsRequest) {
			for (var _i19 = 0; _i19 < ocspReqSimpl.tbsRequest.requestExtensions.length; _i19++) {
				var _row2 = extensionTable.insertRow(extensionTable.rows.length);
				var _cell2 = _row2.insertCell(0);
				// noinspection InnerHTMLJS
				_cell2.innerHTML = ocspReqSimpl.tbsRequest.requestExtensions[_i19].extnID;
			}
	...
	context("OCSP Request Complex Example", () => {
		it("Create And Parse OCSP Request", () => {
			return createOCSPReqInternal().then(() => {
				var asn1 = fromBER(ocspReqBuffer);
				// noinspection JSUnusedLocalSymbols
				var ocspRequest = new OCSPRequest({ schema: asn1.result });
			});
		});
	});
	//*********************************************************************************
