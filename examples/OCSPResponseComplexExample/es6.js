...
import { DistinguishedName } from "../../src";
//<nodewebcryptoossl>
//*********************************************************************************
...
function parseOCSPResp() {
	...
	if (ocspBasicResp.tbsResponseData.responderID instanceof DistinguishedName) {
		for (const rdn of ocspBasicResp.tbsResponseData.responderID) {
			for (let i = 0; i < rdn.typesAndValues.length; i++) {
				let typeval = typemap[rdn.typesAndValues[i].type];
