    ...
    */
	...
	function verifyPDFSignature(buffer) {
		try {
			var _ret = function () {
				...
				var count = 0;
				for (var _i25 = byteRange[0]; _i25 < byteRange[0] + byteRange[1]; _i25++, count++) {
					signedDataView[count] = view[_i25];
				}for (var j = byteRange[2]; j < byteRange[2] + byteRange[3]; j++, count++) {
					signedDataView[count] = view[j];
				}sequence = sequence.then(function () {
					return cmsSignedSimp.verify({
						signer: 0,
				...
				if ("signedAttrs" in cmsSignedSimp.signerInfos[0]) {
					(function () {
						...
						sequence = sequence.then(function (result) {
							...
							for (var _j10 = 0; _j10 < cmsSignedSimp.signerInfos[0].signedAttrs.attributes.length; _j10++) {
								if (cmsSignedSimp.signerInfos[0].signedAttrs.attributes[_j10].type === "1.2.840.113549.1.9.4") {
									messageDigest = cmsSignedSimp.signerInfos[0].signedAttrs.attributes[_j10].values[0].valueBlock.valueHex;
									break;
								}
							}
							...
							for (var _i26 = 0; _i26 < view1.length; _i26++) {
								if (view1[_i26] !== view2[_i26]) return Promise.reject(new Error("Hash is not correct"));
							}
						});
					})();
				}
	...
	function handleFileBrowse(evt) {
		...
		tempReader.onload = function (event) {
			return verifyPDFSignature(event.target.result);
		};
