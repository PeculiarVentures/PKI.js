	...
	function verifyPDFSignature(buffer) {
		try {
			var _ret = function () {
				...
				var count = 0;
				for (var _i24 = byteRange[0]; _i24 < byteRange[0] + byteRange[1]; _i24++, count++) {
					signedDataView[count] = view[_i24];
				...
				if ("signedAttrs" in cmsSignedSimp.signerInfos[0]) {
					(function () {
						...
						sequence = sequence.then(function (result) {
							...
							for (var _i25 = 0; _i25 < view1.length; _i25++) {
								if (view1[_i25] !== view2[_i25]) return Promise.reject(new Error("Hash is not correct"));
							}
						});
					})();
				}
