    ...
    */
			...
			value: function verify() {
				...
				sequence = sequence.then(function () {
					...
					return isEqualBuffer(result, _this55.messageImprint.hashedMessage.valueBlock.valueHex);
				});
				//endregion
   ...
   */
    ...
    */
			...
			value: function sign(privateKey) {
				...
				return crypto.sign(defParams.algorithm, privateKey, new Uint8Array(this.tbs)).then(function (result) {
					...
					_this56.signatureValue = new BitString({ valueHex: result });
				}, function (error) {
					return Promise.reject("Signing error: " + error);
				});
				//endregion
    ...
    */
			...
			value: function verify() {
				...
				sequence = sequence.then(function () {
					//region Get information about public key algorithm and default parameters for import
					var algorithmObject = getAlgorithmByOID(_this57.signature.algorithmId);
					if ("name" in algorithmObject === "") return Promise.reject("Unsupported public key algorithm: " + _this57.signature.algorithmId);
				...
				sequence = sequence.then(function (publicKey) {
					...
					if (publicKey.algorithm.name === "RSA-PSS") {
						...
						try {
							pssParameters = new RSASSAPSSParams({ schema: _this57.signatureAlgorithm.algorithmParams });
						} catch (ex) {
							return Promise.reject(ex);
						}
   ...
   */
    ...
    */
     ...
     */
				...
				return new Sequence({
					name: names.blockName || "",
					value: [new Integer({ name: names.status || "" }), new Sequence({
						optional: true,
						value: [new Repeated({
   ...
   */
    ...
    */
			value: function fromSchema(schema) {
				...
				if ("TimeStampResp.timeStampToken" in asn1.result) this.timeStampToken = new ContentInfo({ schema: asn1.result["TimeStampResp.timeStampToken"] });
				//endregion
			}
			//**********************************************************************************
			/**
    ...
    */
			...
			value: function sign(privateKey, hashAlgorithm) {
				...
				var signed = new ContentInfo({ schema: this.timeStampToken.content });
    ...
    */
			...
			value: function verify() {
				...
				var signed = new SignedData({ schema: this.timeStampToken.content });
    ...
    */
	...
	var tspResponseBuffer = new ArrayBuffer(0); // ArrayBuffer with loaded or created TSP response
	...
	var testData = new Uint8Array([0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09]);
	...
	function createTSPRespInternal() {
		...
		sequence = sequence.then(function () {
			return crypto.digest(hashAlg, testData);
		});
		//endregion

		//region Create specific TST info structure to sign
		sequence = sequence.then(function (result) {
			var hashedBuffer = new ArrayBuffer(4);
			var hashedView = new Uint8Array(hashedBuffer);
			hashedView[0] = 0x7F;
			hashedView[1] = 0x02;
			hashedView[2] = 0x03;
			hashedView[3] = 0x04;

			var tstInfoSimpl = new TSTInfo({
				version: 1,
				policy: "1.1.1",
				messageImprint: new MessageImprint({
					hashAlgorithm: new AlgorithmIdentifier({ algorithmId: getOIDByAlgorithm({ name: hashAlg }) }),
					hashedMessage: new OctetString({ valueHex: result })
				}),
				serialNumber: new Integer({ valueHex: hashedBuffer }),
				genTime: new Date(),
				ordering: true,
				...
				nonce: new Integer({ valueHex: hashedBuffer })
			});
		...
		sequence = sequence.then(function (result) {
			var encapContent = new EncapsulatedContentInfo();
			encapContent.eContentType = "1.2.840.113549.1.9.16.1.4"; // "tSTInfo" content type
			encapContent.eContent = new OctetString({ valueHex: result });
		...
		return sequence.then(function (result) {
			var tspRespSimpl = new TimeStampResp({
	...
	function parseTSPResp() {
		...
		var signedSimpl = new SignedData({ schema: tspRespSimpl.timeStampToken.content });

		var asn1TST = fromBER(signedSimpl.encapContentInfo.eContent.valueBlock.valueHex);
		var tstInfoSimpl = new TSTInfo({ schema: asn1TST.result });
		//endregion
		...
		var hashAlgorithm = dgstmap[tstInfoSimpl.messageImprint.hashAlgorithm.algorithmId];
		if (typeof hashAlgorithm === "undefined") hashAlgorithm = tstInfoSimpl.messageImprint.hashAlgorithm.algorithmId;

		var imprintTable = document.getElementById("resp-imprint");
		...
		if ("accuracy" in tstInfoSimpl) {
			var accuracyTable = document.getElementById("resp-accuracy");

			var rowInner = accuracyTable.insertRow(accuracyTable.rows.length);
		...
		if ("tsa" in tstInfoSimpl) {
			switch (tstInfoSimpl.tsa.type) {
				...
				case 7:
					// iPAddress
					{
						var view = new Uint8Array(tstInfoSimpl.tsa.value.valueBlock.valueHex);
				...
				case 4:
					// directoryName
					{
						...
						var rdnTable = document.getElementById("resp-tsa");

						for (var i = 0; i < tstInfoSimpl.tsa.value.typesAndValues.length; i++) {
							var typeval = rdnmap[tstInfoSimpl.tsa.value.typesAndValues[i].type];
							if (typeof typeval === "undefined") typeval = tstInfoSimpl.tsa.value.typesAndValues[i].type;

							var subjval = tstInfoSimpl.tsa.value.typesAndValues[i].value.valueBlock.value;

							var _rowInner = rdnTable.insertRow(rdnTable.rows.length);
							var _cell0Inner = _rowInner.insertCell(0);
							_cell0Inner.innerHTML = typeval;
							var _cell1Inner = _rowInner.insertCell(1);
							_cell1Inner.innerHTML = subjval;
						}
		...
		if ("extensions" in tstInfoSimpl) {
			...
			for (var _i24 = 0; _i24 < tstInfoSimpl.extensions.length; _i24++) {
				var _rowInner2 = extensionTable.insertRow(extensionTable.rows.length);
				var _cell0Inner2 = _rowInner2.insertCell(0);
				_cell0Inner2.innerHTML = tstInfoSimpl.extensions[_i24].extnID;
			}
	...
	//*********************************************************************************
	context("TSP Response Complex Example", function () {
		//region Initial variables
		...
		signAlgs.forEach(function (_signAlg) {
			hashAlgs.forEach(function (_hashAlg) {
				...
				it(testName, function () {
					...
					return createTSPRespInternal().then(function () {
						var asn1 = fromBER(tspResponseBuffer);
						var tspResponse = new TimeStampResp({ schema: asn1.result });

						return verifyTSPRespInternal().then(function (result) {
