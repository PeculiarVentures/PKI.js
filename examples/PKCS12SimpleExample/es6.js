...
context("PKCS#12 Simple Example", () =>
{
	...
	it("Password-based Integrity, SHA-1", async () => passwordBasedIntegrityInternal(password, "SHA-1"));
	it("Password-based Integrity, SHA-256", async () => passwordBasedIntegrityInternal(password, "SHA-256"));
	it("Password-based Integrity, SHA-384", async () => passwordBasedIntegrityInternal(password, "SHA-384"));
	it("Password-based Integrity, SHA-512", async () => passwordBasedIntegrityInternal(password, "SHA-512"));
	it("Password-based Integrity, incorrect algorithm", async () => passwordBasedIntegrityInternal(password, "SHA-5122").then(() => Promise.reject("Error must be here"), () => Promise.resolve()));

	it("Certificate-based Integrity", async () => certificateBasedIntegrityInternal());

	it("No-Privacy Test", async () => noPrivacyInternal(password));

	it("Password Privacy", async () => passwordPrivacyInternal(password));

	it("Certificate Privacy", async () => certificatePrivacyInternal(password));

	it("Making OpenSSL-like PKCS#12 Data", async () => openSSLLikeInternal(password).then(result => parsePKCS12Internal(result, password)));
});
//**********************************************************************************
