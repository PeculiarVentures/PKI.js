...
(function () {
  ...
  class TimeStampReq {
    ...
    toSchema() {
      ...
      if ("certReq" in this && TimeStampReq.compareWithDefault("certReq", this.certReq) === false) outputArray.push(new Boolean({
        value: this.certReq
  ...
  function createTSPReqInternal() {
    //region Initial variables
    var tspReqSimpl = new TimeStampReq(); //endregion
    ...
    tspReqSimpl.nonce = new Integer({
      valueHex: fictionBuffer
    }); //endregion
    //region Encode TSP request and put on the Web page

    tspReqBuffer = tspReqSimpl.toSchema().toBER(false); //endregion
  ...
  function createTSPReq() {
    return Promise.resolve().then(() => createTSPReqInternal()).then(() => {
      ...
      resultString = "".concat(resultString).concat(formatPEM(toBase64(arrayBufferToString(tspReqBuffer))));
  ...
  function parseTSPReq() {
    ...
    cell1.innerHTML = bufferToHexCodes(tspReqSimpl.messageImprint.hashedMessage.valueBlock.valueHex); //endregion 
    ...
    if ("extensions" in tspReqSimpl) {
      ...
      for (var i = 0; i < tspReqSimpl.extensions.length; i++) {
        ...
        var cell0Inner = rowInner.insertCell(0); // noinspection InnerHTMLJS
  ...
  context("TSP Request Complex Example", () => {
    it("Create And Parse TSP Request", () => {
      return createTSPReqInternal().then(() => {
        var asn1 = fromBER(tspReqBuffer); // noinspection JSUnusedLocalSymbols

        var tspRequest = new TimeStampReq({
          schema: asn1.result
        });
      });
    });
