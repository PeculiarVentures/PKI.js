...
(function () {
  ...
  function createPKCS10Internal() {
    ...
    sequence = sequence.then(() => pkcs10.subjectPublicKeyInfo.importKey(publicKey)); //endregion
    ...
    sequence = sequence.then(() => pkcs10.sign(privateKey, hashAlg), error => Promise.reject("Error during exporting public key: ".concat(error))); //endregion

    return sequence.then(() => {
      pkcs10Buffer = pkcs10.toSchema().toBER(false);
    }, error => Promise.reject("Error signing PKCS#10: ".concat(error)));
  } //*********************************************************************************
  ...
  function createPKCS10() {
    return Promise.resolve().then(() => createPKCS10Internal()).then(() => {
      ...
      resultString = "".concat(resultString).concat(formatPEM(toBase64(arrayBufferToString(pkcs10Buffer))));
  ...
  function parsePKCS10() {
    ...
    document.getElementById("pkcs10-subject").innerHTML = ""; // noinspection InnerHTMLJS

    document.getElementById("pkcs10-exten").innerHTML = "";
    document.getElementById("pkcs10-data-block").style.display = "none";
    document.getElementById("pkcs10-attributes").style.display = "none"; //endregion
    ...
    for (var i = 0; i < pkcs10.subject.typesAndValues.length; i++) {
      ...
      var ulrow = "<li><p><span>".concat(typeval, "</span> ").concat(subjval, "</p></li>"); // noinspection InnerHTMLJS
    ...
    if (pkcs10.subjectPublicKeyInfo.algorithm.algorithmId.indexOf("1.2.840.113549") !== -1) {
      ...
      var rsaPublicKeySimple = new RSAPublicKey({
    ...
    document.getElementById("keysize").innerHTML = publicKeySize; //endregion
    ...
    if (typeof signatureAlgorithm === "undefined") signatureAlgorithm = pkcs10.signatureAlgorithm.algorithmId;else signatureAlgorithm = "".concat(signatureAlgorithm, " (").concat(pkcs10.signatureAlgorithm.algorithmId, ")"); // noinspection InnerHTMLJS
    ...
    if ("attributes" in pkcs10) {
      for (var _i34 = 0; _i34 < pkcs10.attributes.length; _i34++) {
        var _typeval = pkcs10.attributes[_i34].type;
        var _subjval = "";

        for (var j = 0; j < pkcs10.attributes[_i34].values.length; j++) {
          // noinspection OverlyComplexBooleanExpressionJS
          if (pkcs10.attributes[_i34].values[j] instanceof Utf8String || pkcs10.attributes[_i34].values[j] instanceof BmpString || pkcs10.attributes[_i34].values[j] instanceof UniversalString || pkcs10.attributes[_i34].values[j] instanceof NumericString || pkcs10.attributes[_i34].values[j] instanceof PrintableString || pkcs10.attributes[_i34].values[j] instanceof TeletexString || pkcs10.attributes[_i34].values[j] instanceof VideotexString || pkcs10.attributes[_i34].values[j] instanceof IA5String || pkcs10.attributes[_i34].values[j] instanceof GraphicString || pkcs10.attributes[_i34].values[j] instanceof VisibleString || pkcs10.attributes[_i34].values[j] instanceof GeneralString || pkcs10.attributes[_i34].values[j] instanceof CharacterString) {
            _subjval = _subjval + (_subjval.length === 0 ? "" : ";") + pkcs10.attributes[_i34].values[j].valueBlock.value;
          } else _subjval = _subjval + (_subjval.length === 0 ? "" : ";") + pkcs10.attributes[_i34].values[j].constructor.blockName();
        }

        var _ulrow = "<li><p><span>".concat(_typeval, "</span> ").concat(_subjval, "</p></li>"); // noinspection InnerHTMLJS
  ...
  function verifyPKCS10Internal() {
    ...
    return pkcs10.verify(); //endregion
  } //*********************************************************************************
      ...
      hashAlgs.forEach(_hashAlg => {
        ...
        it(testName, () => {
          ...
          return createPKCS10Internal().then(() => {
            ...
            assert.equal(pkcs10.signatureAlgorithm.algorithmId, algorithmsMap.get(testName), "Signature algorithm must be ".concat(testName));
