   ...
   */
    ...
    */
			...
			value: function toSchema() {
				//region Construct and return new ASN.1 schema for this object
				return new Sequence({
					value: [new ObjectIdentifier({ value: this.responseType }), this.response]
				});
				//endregion
			}
			//**********************************************************************************
    ...
    */
     ...
     */
				...
				return new Sequence({
					name: names.blockName || "",
					value: [new ObjectIdentifier({ name: names.responseType || "" }), new OctetString({ name: names.response || "" })]
				});
			}
		}]);
   ...
   */
    ...
    */
			...
			value: function createForCertificate(certificate, parameters) {
				...
				sequence = sequence.then(function (result) {
					_this55.issuerNameHash = new OctetString({ valueHex: result });
					...
					_this55.issuerKeyHash = new OctetString({ valueHex: result });
				}, function (error) {
					return Promise.reject(error);
				});
				//endregion
    ...
    */
			...
			value: function verify() {
				...
				sequence = sequence.then(function () {
					...
					var anyPolicyArray = new Array(_this56.certs.length - 1); // Minus "trusted anchor"
					for (var ii = 0; ii < _this56.certs.length - 1; ii++) {
						anyPolicyArray[ii] = true;
					}policiesAndCerts.push(anyPolicyArray);

					var policyMappings = new Array(_this56.certs.length - 1); // Array of "PolicyMappings" for each certificate
					var certPolicies = new Array(_this56.certs.length - 1); // Array of "CertificatePolicies" for each certificate

					var explicitPolicyStart = explicitPolicyIndicator ? _this56.certs.length - 1 : -1;
					//endregion

					//region Gather all neccessary information from certificate chain
					for (var i = _this56.certs.length - 2; i >= 0; i--, pathDepth++) {
						if ("extensions" in _this56.certs[i]) {
							//region Get information about certificate extensions
							for (var j = 0; j < _this56.certs[i].extensions.length; j++) {
								//region CertificatePolicies
								if (_this56.certs[i].extensions[j].extnID === "2.5.29.32") {
									certPolicies[i] = _this56.certs[i].extensions[j].parsedValue;
									...
									for (var k = 0; k < _this56.certs[i].extensions[j].parsedValue.certificatePolicies.length; k++) {
										var policyIndex = -1;

										//region Try to find extension in "allPolicies" array
										for (var _s = 0; _s < allPolicies.length; _s++) {
											if (_this56.certs[i].extensions[j].parsedValue.certificatePolicies[k].policyIdentifier === allPolicies[_s]) {
												policyIndex = _s;
												break;
											}
										}
										...
										if (policyIndex === -1) {
											allPolicies.push(_this56.certs[i].extensions[j].parsedValue.certificatePolicies[k].policyIdentifier);

											var certArray = new Array(_this56.certs.length - 1);
											certArray[i] = true;
								...
								if (_this56.certs[i].extensions[j].extnID === "2.5.29.33") {
									...
									policyMappings[i] = _this56.certs[i].extensions[j].parsedValue;
								}
								//endregion

								//region PolicyConstraints
								if (_this56.certs[i].extensions[j].extnID === "2.5.29.36") {
									if (explicitPolicyIndicator === false) {
										//region requireExplicitPolicy
										if (_this56.certs[i].extensions[j].parsedValue.requireExplicitPolicy === 0) {
											explicitPolicyIndicator = true;
											explicitPolicyStart = i;
										} else {
											if (pendingConstraints[0] === false) {
												pendingConstraints[0] = true;
												explicitPolicyPending = _this56.certs[i].extensions[j].parsedValue.requireExplicitPolicy;
											} else explicitPolicyPending = explicitPolicyPending > _this56.certs[i].extensions[j].parsedValue.requireExplicitPolicy ? _this56.certs[i].extensions[j].parsedValue.requireExplicitPolicy : explicitPolicyPending;
										}
										//endregion

										//region inhibitPolicyMapping
										if (_this56.certs[i].extensions[j].parsedValue.inhibitPolicyMapping === 0) policyMappingInhibitIndicator = true;else {
											if (pendingConstraints[1] === false) {
												pendingConstraints[1] = true;
												policyMappingInhibitPending = _this56.certs[i].extensions[j].parsedValue.inhibitPolicyMapping + 1;
											} else policyMappingInhibitPending = policyMappingInhibitPending > _this56.certs[i].extensions[j].parsedValue.inhibitPolicyMapping + 1 ? _this56.certs[i].extensions[j].parsedValue.inhibitPolicyMapping + 1 : policyMappingInhibitPending;
										}
										//endregion
									}
								}
								...
								if (_this56.certs[i].extensions[j].extnID === "2.5.29.54") {
									if (inhibitAnyPolicyIndicator === false) {
										if (_this56.certs[i].extensions[j].parsedValue.valueBlock.valueDec === 0) inhibitAnyPolicyIndicator = true;else {
											if (pendingConstraints[2] === false) {
												pendingConstraints[2] = true;
												inhibitAnyPolicyPending = _this56.certs[i].extensions[j].parsedValue.valueBlock.valueDec;
											} else inhibitAnyPolicyPending = inhibitAnyPolicyPending > _this56.certs[i].extensions[j].parsedValue.valueBlock.valueDec ? _this56.certs[i].extensions[j].parsedValue.valueBlock.valueDec : inhibitAnyPolicyPending;
										}
									}
								}
								//endregion
					...
					for (var _i17 = 0; _i17 < _this56.certs.length - 1; _i17++) {
						//region Check that there is "policy mapping" for level "i + 1"
						if (_i17 < _this56.certs.length - 2 && typeof policyMappings[_i17 + 1] !== "undefined") {
					...
					for (var _i19 = 0; _i19 < policiesAndCerts.length; _i19++) {
						...
						for (var _j4 = 0; _j4 < _this56.certs.length - 1; _j4++) {
					...
					for (var i = _this56.certs.length - 2; i >= 0; i--, pathDepth++) {
						...
						if ("extensions" in _this56.certs[i]) {
							for (var j = 0; j < _this56.certs[i].extensions.length; j++) {
								...
								if (_this56.certs[i].extensions[j].extnID === "2.5.29.30") {
									if ("permittedSubtrees" in _this56.certs[i].extensions[j].parsedValue) certPermittedSubtrees = certPermittedSubtrees.concat(_this56.certs[i].extensions[j].parsedValue.permittedSubtrees);

									if ("excludedSubtrees" in _this56.certs[i].extensions[j].parsedValue) certExcludedSubtrees = certExcludedSubtrees.concat(_this56.certs[i].extensions[j].parsedValue.excludedSubtrees);
								}
								//endregion

								//region SubjectAltName
								if (_this56.certs[i].extensions[j].extnID === "2.5.29.17") subjectAltNames = subjectAltNames.concat(_this56.certs[i].extensions[j].parsedValue.altNames);
								//endregion
							}
						}
						...
						for (var _j6 = 0; _j6 < requiredNameForms.length; _j6++) {
							switch (requiredNameForms[_j6].base.type) {
								case 4:
									// directoryName
									{
										if (requiredNameForms[_j6].base.value.typesAndValues.length !== _this56.certs[i].subject.typesAndValues.length) continue;

										formFound = true;

										for (var k = 0; k < _this56.certs[i].subject.typesAndValues.length; k++) {
											if (_this56.certs[i].subject.typesAndValues[k].type !== requiredNameForms[_j6].base.value.typesAndValues[k].type) {
												formFound = false;
												break;
											}
										}
								...
								switch (p) {
									...
									case 0:
										if (subjectAltNames.length > 0) {
											...
											{
												for (var _k4 = 0; _k4 < _this56.certs[i].subject.typesAndValues.length; _k4++) {
													if (_this56.certs[i].subject.typesAndValues[_k4].type === "1.2.840.113549.1.9.1" || // PKCS#9 e-mail address
													_this56.certs[i].subject.typesAndValues[_k4].type === "0.9.2342.19200300.100.1.3") // RFC1274 "rfc822Mailbox" e-mail address
														{
															valueExists = true;
															groupPermitted = groupPermitted || compareRFC822Name(_this56.certs[i].subject.typesAndValues[_k4].value.valueBlock.value, group[_j8].base.value);
														}
												}
											}
										break;
									...
									case 2:
										valueExists = true;
										groupPermitted = compareDirectoryName(_this56.certs[i].subject, group[_j8].base.value);
										break;
									//endregion
									//region uniformResourceIdentifier
									case 3:
							...
							switch (excludedSubtrees[_j9].base.type) {
								...
								case 1:
									if (subjectAltNames.length >= 0) {
										...
										{
											for (var _k9 = 0; _k9 < _this56.subject.typesAndValues.length; _k9++) {
												if (_this56.subject.typesAndValues[_k9].type === "1.2.840.113549.1.9.1" || // PKCS#9 e-mail address
												_this56.subject.typesAndValues[_k9].type === "0.9.2342.19200300.100.1.3") // RFC1274 "rfc822Mailbox" e-mail address
													excluded = excluded || compareRFC822Name(_this56.subject.typesAndValues[_k9].value.valueBlock.value, excludedSubtrees[_j9].base.value);
											}
										}
									break;
								//endregion
								...
								//region directoryName
								case 4:
									excluded = excluded || compareDirectoryName(_this56.certs[i].subject, excludedSubtrees[_j9].base.value);
									break;
								//endregion
								//region uniformResourceIdentifier
								case 6:
   ...
   */
    ...
    */
			...
			value: function getCertificateStatus(certificate, issuerCertificate) {
				...
				try {
					for (var _iterator14 = this.tbsResponseData[Symbol.iterator](), _step14; !(_iteratorNormalCompletion14 = (_step14 = _iterator14.next()).done); _iteratorNormalCompletion14 = true) {
						var response = _step14.value;
				...
				sequence = sequence.then(function () {
					...
					try {
						for (var _iterator15 = _this57.tbsResponseData.responses[Symbol.iterator](), _step15; !(_iteratorNormalCompletion15 = (_step15 = _iterator15.next()).done); _iteratorNormalCompletion15 = true) {
							var response = _step15.value;
							var _iteratorNormalCompletion16 = true;
							var _didIteratorError16 = false;
							var _iteratorError16 = undefined;

							try {
								for (var _iterator16 = certIDs[Symbol.iterator](), _step16; !(_iteratorNormalCompletion16 = (_step16 = _iterator16.next()).done); _iteratorNormalCompletion16 = true) {
    ...
    */
			...
			value: function sign(privateKey, hashAlgorithm) {
				...
				return crypto.sign(defParams.algorithm, privateKey, new Uint8Array(this.tbsResponseData.tbs)).then(function (result) {
					...
					_this58.signature = new BitString({ valueHex: result });
				}, function (error) {
					return Promise.reject("Signing error: " + error);
				});
				//endregion
    ...
    */
				...
				if (responderType === 0) // By Name
					{
						sequence = sequence.then(function () {
							...
							try {
								for (var _iterator17 = _this59.certs.entries()[Symbol.iterator](), _step17; !(_iteratorNormalCompletion17 = (_step17 = _iterator17.next()).done); _iteratorNormalCompletion17 = true) {
									var _step17$value = _slicedToArray(_step17.value, 2);

									var index = _step17$value[0];
									var certificate = _step17$value[1];
					...
					{
						sequence = sequence.then(function () {
							return Promise.all(Array.from(_this59.certs, function (element) {
								...
								try {
									for (var _iterator18 = _this59.certs.entries()[Symbol.iterator](), _step18; !(_iteratorNormalCompletion18 = (_step18 = _iterator18.next()).done); _iteratorNormalCompletion18 = true) {
										var _step18$value = _slicedToArray(_step18.value, 2);

										var index = _step18$value[0];
										var certificate = _step18$value[1];
     ...
     */
				function checkCA(cert) {
					...
					try {
						for (var _iterator19 = cert.extensions[Symbol.iterator](), _step19; !(_iteratorNormalCompletion19 = (_step19 = _iterator19.next()).done); _iteratorNormalCompletion19 = true) {
				...
				sequence = sequence.then(function () {
					...
					signerCert = _this59.certs[certIndex];
					...
					return Promise.all(Array.from(_this59.certs, function (element) {
						...
						try {
							for (var _iterator20 = promiseResults[Symbol.iterator](), _step20; !(_iteratorNormalCompletion20 = (_step20 = _iterator20.next()).done); _iteratorNormalCompletion20 = true) {
								var promiseResult = _step20.value;
				...
				sequence = sequence.then(function () {
					//region Get information about public key algorithm and default parameters for import
					var algorithmId = void 0;
					if (_this59.certs[certIndex].signatureAlgorithm.algorithmId === "1.2.840.113549.1.1.10") algorithmId = _this59.certs[certIndex].signatureAlgorithm.algorithmId;else algorithmId = _this59.certs[certIndex].subjectPublicKeyInfo.algorithm.algorithmId;
					...
					if (algorithmName === "ECDSA") {
						...
						if (_this59.certs[certIndex].subjectPublicKeyInfo.algorithm.algorithmParams instanceof ObjectIdentifier === false) return Promise.reject("Incorrect type for ECDSA public key parameters");

						var curveObject = getAlgorithmByOID(_this59.certs[certIndex].subjectPublicKeyInfo.algorithm.algorithmParams.valueBlock.toString());
						if ("name" in curveObject === false) return Promise.reject("Unsupported named curve algorithm: " + _this59.certs[certIndex].subjectPublicKeyInfo.algorithm.algorithmParams.valueBlock.toString());
						//endregion
					...
					var publicKeyInfoSchema = _this59.certs[certIndex].subjectPublicKeyInfo.toSchema();
					var publicKeyInfoBuffer = publicKeyInfoSchema.toBER(false);
					var publicKeyInfoView = new Uint8Array(publicKeyInfoBuffer);
				...
				sequence = sequence.then(function (publicKey) {
					...
					//region Special case for ECDSA signatures
					var signatureValue = _this59.signature.valueBlock.valueHex;
					...
					if (publicKey.algorithm.name === "RSA-PSS") {
						...
						try {
							pssParameters = new RSASSAPSSParams({ schema: _this59.signatureAlgorithm.algorithmParams });
						} catch (ex) {
							return Promise.reject(ex);
						}
   ...
   */
    ...
    */
			value: function fromSchema(schema) {
				...
				if ("responseBytes" in asn1.result) this.responseBytes = new ResponseBytes({ schema: asn1.result.responseBytes });
				//endregion
			}
			//**********************************************************************************
			/**
    ...
    */
			...
			value: function getCertificateStatus(certificate, issuerCertificate) {
				...
				try {
					var asn1Basic = fromBER(this.responseBytes.response.valueBlock.valueHex);
    ...
    */
			...
			value: function sign(privateKey, hashAlgorithm) {
				...
				if (this.responseBytes.responseType === "1.3.6.1.5.5.7.48.1.1") {
					var asn1 = fromBER(this.responseBytes.response.valueBlock.valueHex);
					var basicResponse = new BasicOCSPResponse({ schema: asn1.result });
    ...
    */
     ...
     */
				...
				return new Sequence({
					...
					value: [new Enumerated({ name: names.responseStatus || "responseStatus" }), new Constructed({
						optional: true,
						idBlock: {
							tagClass: 3, // CONTEXT-SPECIFIC
							tagNumber: 0 // [0]
	...
	var ocspResponseBuffer = new ArrayBuffer(0); // ArrayBuffer with loaded or created OCSP response
	...
	function createOCSPRespInternal() {
		...
		var ocspRespSimpl = new OCSPResponse();
		var ocspBasicResp = new BasicOCSPResponse();
		...
		return sequence.then(function () {
			var encodedOCSPBasicResp = ocspBasicResp.toSchema().toBER(false);
	...
	function parseOCSPResp() {
		...
		var asn1 = fromBER(ocspResponseBuffer);
		var ocspRespSimpl = new OCSPResponse({ schema: asn1.result });
		//endregion 
		...
		if ("responseBytes" in ocspRespSimpl) {
			var asn1Basic = fromBER(ocspRespSimpl.responseBytes.response.valueBlock.valueHex);
		...
		var signatureAlgorithm = algomap[ocspBasicResp.signatureAlgorithm.algorithmId];
		if (typeof signatureAlgorithm === "undefined") signatureAlgorithm = ocspBasicResp.signatureAlgorithm.algorithmId;else signatureAlgorithm = signatureAlgorithm + " (" + ocspBasicResp.signatureAlgorithm.algorithmId + ")";
		...
		if (ocspBasicResp.tbsResponseData.responderID instanceof RelativeDistinguishedNames) {
			...
			for (var i = 0; i < ocspBasicResp.tbsResponseData.responderID.typesAndValues.length; i++) {
				var typeval = typemap[ocspBasicResp.tbsResponseData.responderID.typesAndValues[i].type];
				if (typeof typeval === "undefined") typeval = ocspBasicResp.tbsResponseData.responderID.typesAndValues[i].type;

				var subjval = ocspBasicResp.tbsResponseData.responderID.typesAndValues[i].value.valueBlock.value;
		...
		if ("responseExtensions" in ocspBasicResp) {
			...
			for (var _i24 = 0; _i24 < ocspBasicResp.responseExtensions.length; _i24++) {
				var _typeval = extenmap[ocspBasicResp.responseExtensions[_i24].extnID];
				if (typeof _typeval === "undefined") _typeval = ocspBasicResp.responseExtensions[_i24].extnID;

				var _row = extensionTable.insertRow(extensionTable.rows.length);
				var _cell = _row.insertCell(0);
				_cell.innerHTML = _typeval;
			}
		...
		for (var _i25 = 0; _i25 < ocspBasicResp.tbsResponseData.responses.length; _i25++) {
			var _typeval2 = bufferToHexCodes(ocspBasicResp.tbsResponseData.responses[_i25].certID.serialNumber.valueBlock.valueHex);
			var _subjval = "";

			switch (ocspBasicResp.tbsResponseData.responses[_i25].certStatus.idBlock.tag_number) {
				case 0:
					_subjval = "good";
					break;
				case 1:
			...
			var _row2 = responsesTable.insertRow(responsesTable.rows.length);
			var _cell2 = _row2.insertCell(0);
			_cell2.innerHTML = _typeval2;
			var _cell3 = _row2.insertCell(1);
			_cell3.innerHTML = _subjval;
		}
		//endregion 
	...
	function verifyOCSPRespInternal() {
		...
		if ("responseBytes" in ocspRespSimpl) {
			var asn1Basic = fromBER(ocspRespSimpl.responseBytes.response.valueBlock.valueHex);
			ocspBasicResp = new BasicOCSPResponse({ schema: asn1Basic.result });
		} else return Promise.reject("No \"ResponseBytes\" in the OCSP Response - nothing to verify");
		//endregion
	...
	//*********************************************************************************
	context("OCSP Response Complex Example", function () {
		//region Initial variables
		...
		signAlgs.forEach(function (_signAlg) {
			hashAlgs.forEach(function (_hashAlg) {
				...
				it(testName, function () {
					...
					return createOCSPRespInternal().then(function () {
						var asn1 = fromBER(ocspResponseBuffer);
						var ocspResponse = new OCSPResponse({ schema: asn1.result });

						return verifyOCSPRespInternal().then(function (result) {
