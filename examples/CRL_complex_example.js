...
(function () {
  ...
  function createCRLInternal() {
    ...
    var crlSimpl = new CertificateRevocationList();
    crlSimpl.version = 1;
    ...
    sequence = sequence.then(() => crlSimpl.sign(privateKey, hashAlg)); //endregion
  ...
  function createCRL() {
    return Promise.resolve().then(() => createCRLInternal()).then(() => {
      var resultString = "-----BEGIN X509 CRL-----\r\n";
  ...
  function parseCRL() {
    ...
    for (var i = 0; i < crlSimpl.issuer.typesAndValues.length; i++) {
      var typeval = rdnmap[crlSimpl.issuer.typesAndValues[i].type];

      if (typeof typeval === "undefined") {
        typeval = crlSimpl.issuer.typesAndValues[i].type;
      }

      var subjval = crlSimpl.issuer.typesAndValues[i].value.valueBlock.value;
    ...
    document.getElementById("crl-this-update").innerHTML = crlSimpl.thisUpdate.value.toString(); //endregion
    ...
    var signatureAlgorithm = algomap[crlSimpl.signature.algorithmId];
    if (typeof signatureAlgorithm === "undefined") signatureAlgorithm = crlSimpl.signature.algorithmId;else signatureAlgorithm = "".concat(signatureAlgorithm, " (").concat(crlSimpl.signature.algorithmId, ")"); // noinspection InnerHTMLJS

    document.getElementById("crl-sign-algo").innerHTML = signatureAlgorithm; //endregion
    //region Put information about revoked certificates

    if ("revokedCertificates" in crlSimpl) {
      for (var _i34 = 0; _i34 < crlSimpl.revokedCertificates.length; _i34++) {
        var _row = revokedTable.insertRow(revokedTable.rows.length);

        var _cell = _row.insertCell(0); // noinspection InnerHTMLJS


        _cell.innerHTML = bufferToHexCodes(crlSimpl.revokedCertificates[_i34].userCertificate.valueBlock.valueHex);

        var _cell2 = _row.insertCell(1); // noinspection InnerHTMLJS


        _cell2.innerHTML = crlSimpl.revokedCertificates[_i34].revocationDate.value.toString();
      }
    ...
    if ("crlExtensions" in crlSimpl) {
      for (var _i35 = 0; _i35 < crlSimpl.crlExtensions.extensions.length; _i35++) {
        var _row2 = extensionTable.insertRow(extensionTable.rows.length);

        var _cell3 = _row2.insertCell(0); // noinspection InnerHTMLJS


        _cell3.innerHTML = crlSimpl.crlExtensions.extensions[_i35].extnID;
      }
  ...
  function verifyCRLInternal() {
    //region Initial check
    if (crlBuffer.byteLength === 0) return Promise.reject("Nothing to verify");
    if (issuerCertificate === null && issuerPublicKey === null) return Promise.reject("Load CRL's issuer certificate"); //endregion
    //region Decode existing CRL
    ...
    var verifyObject = {};
    if (issuerCertificate !== null) verifyObject.issuerCertificate = issuerCertificate;
    if (issuerPublicKey !== null) verifyObject.publicKeyInfo = issuerPublicKey;
    return crlSimpl.verify(verifyObject); //endregion
  } //*********************************************************************************
  ...
  function handleIssuerCert(evt) {
    ...
    tempReader.onload = event => {
      issuerPublicKey = null; // noinspection JSUnresolvedVariable
      ...
      hashAlgs.forEach(_hashAlg => {
        ...
        it(testName, () => {
          ...
          return createCRLInternal().then(() => {
            ...
            assert.equal(crl.signatureAlgorithm.algorithmId, algorithmsMap.get(testName), "Signature algorithm must be ".concat(testName));
