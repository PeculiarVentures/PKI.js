  ...
  */
   ...
   */
	...
	function parseCRL() {
		...
		if (typeof signatureAlgorithm === "undefined") signatureAlgorithm = crlSimpl.signature.algorithmId;else signatureAlgorithm = `${signatureAlgorithm} (${crlSimpl.signature.algorithmId})`;
		...
		if ("revokedCertificates" in crlSimpl) {
			for (var _i18 = 0; _i18 < crlSimpl.revokedCertificates.length; _i18++) {
				var _row = revokedTable.insertRow(revokedTable.rows.length);
				var _cell = _row.insertCell(0);
				// noinspection InnerHTMLJS
				_cell.innerHTML = bufferToHexCodes(crlSimpl.revokedCertificates[_i18].userCertificate.valueBlock.valueHex);
				var _cell2 = _row.insertCell(1);
				// noinspection InnerHTMLJS
				_cell2.innerHTML = crlSimpl.revokedCertificates[_i18].revocationDate.value.toString();
			}
		...
		if ("crlExtensions" in crlSimpl) {
			for (var _i19 = 0; _i19 < crlSimpl.crlExtensions.extensions.length; _i19++) {
				var _row2 = extensionTable.insertRow(extensionTable.rows.length);
				var _cell3 = _row2.insertCell(0);
				// noinspection InnerHTMLJS
				_cell3.innerHTML = crlSimpl.crlExtensions.extensions[_i19].extnID;
			}
	...
	function verifyCRLInternal() {
		...
		if (issuerCertificate === null && issuerPublicKey === null) return Promise.reject("Load CRL's issuer certificate");
		//endregion
